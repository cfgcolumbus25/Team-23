...
export default function AdminEmailsPage() {
  ...
  async function fetchInstitutions() {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/admin/institutions?${queryParams}`, {
        headers: {
  ...
  async function sendEmails() {
    if (selectedInstitutions.size === 0) {
      setMessage({ type: 'error', text: 'Please select at least one institution' })
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        setMessage({
          type: 'success',
          text: `Successfully sent ${data.sent_count} emails. ${data.failed_count} failed.`,
        })
        setSelectedInstitutions(new Set())
        fetchEmailHistory()
      } else {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              {sending ? 'Sending...' : `Send Emails (${selectedInstitutions.size})`}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden mb-8">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          >
            {selectedInstitutions.size === institutions.length ? 'Deselect All' : 'Select All'}
          </button>
        </div>
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-500">No institutions found</div>
          ) : (
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Recent Emails Sent</h2>
        </div>
        <div className="overflow-x-auto">
